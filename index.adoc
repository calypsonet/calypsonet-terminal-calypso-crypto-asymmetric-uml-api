...
| Throws      | <<type_AsymmetricCrypto,`AsymmetricCrypto`>> -- if the provided public key value is not compliant with the current elliptic curve, or if an error occurs during the initialisation.
|===
...
| Throws      | <<type_AsymmetricCrypto,`AsymmetricCrypto`>> -- if an error occurs while verifying the signature.
|===
...
| Throws      | <<type_AsymmetricCrypto,`AsymmetricCrypto`>> -- if an error occurs while updating the session.
|===
...
| Throws      | <<type_CertificateValidation,`CertificateValidation`>> -- if the provided certificate has an unsupported format.
|===
